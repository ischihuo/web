...
async function updateArticle(ctx) {
    ...
    if(!err){
        ...
        if(data.id > 0){
            ...
            array.push(data.id);
            const [result] = await connection.execute('UPDATE `article` SET `title`=?,`description`=?,`read_type`=?,`sort_id`=?,`content`=?,`article_extend`=?,`art_img`=? where `id`=?', array);
            ...
            const [result] = await connection.execute('INSERT INTO `article` (title,description,read_type,sort_id,content,article_extend,art_img,create_time,passed,user_id) VALUES (?,?,?,?,?,?,?,?,?,?)', array);
...
//获取图片详情
async function getImgById(ctx) {
    const data = ctx.request.body;
    let id = data.id >> 0;
    let msg;
    const connection = await mysql.createConnection(config.mysqlDB);
    const [list] = await connection.execute("SELECT a.* FROM img as a  where a.id=?", [id]);
    const obj = list[0];
    if(list.length === 1){
        const user = ctx.state.userInfo;
		obj.xx = JSON.stringify(user);
    ...
    //扩展上一条下一条数据
    let [prev] = await connection.execute("SELECT `id`,`img_title` FROM img where id<? order by id desc limit 1", [id]);
    let [next] = await connection.execute("SELECT `id`,`img_title` FROM img where id>? order by id asc limit 1", [id]);
    obj.prev = prev.length?prev[0]:{};
    obj.next = next.length?next[0]:{};
    await connection.end();
    ctx.body = {
...
async function updateImg(ctx) {
    ...
    const array = [
        data.img_title.slice(0,100),
        data.img_description.slice(0,255),
        data.img_path,
        data.img_href
    ];
    if(!err){
        ...
        if(data.id > 0){
            array.push(data.id);
            const [result] = await connection.execute('UPDATE `img` SET `img_title`=?,`img_description`=?,`img_path`=?,`img_href`=? where `id`=?', array);
            err = result.affectedRows === 1 ? '' :'图片修改失败';
        }else{
            //添加文章
            array.push(new Date().toLocaleString());//添加日期
            const [result] = await connection.execute('INSERT INTO `img` (img_title,img_description,img_path,img_href,create_time) VALUES (?,?,?,?,?)', array);
            err = result.affectedRows === 1 ? '' :'文章添加失败';
        }
        await connection.end();
    }
...
async function deleteImg(ctx){
    ...
    if(/^\d+(,\d+)*$/.test(ids)){
        const arr = ids.split(',');
        let sql = `DELETE from img where id in (${arr.map(() => '?').join(',')})`;
        const user = ctx.state.userInfo;
        if(user.user_type > 2){
            sql += " and user_id=?";//会员只能删除自己的
            arr.push(user.id >> 0);
        ...
        const connection = await mysql.createConnection(config.mysqlDB);
        const [result] = await connection.execute(sql, arr);
        msg = result.affectedRows > 0 ? '':'删除图片失败！';
        await connection.end();
    }else{
        msg = 'ID参数不合法';
    }
