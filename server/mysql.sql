...
CREATE TABLE `img` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '主键',
  `img_title` char(100) NOT NULL DEFAULT '' COMMENT '图片名称',
  `img_description` char(255) NOT NULL DEFAULT '' COMMENT '图片描述',
  `img_path` char(200) NOT NULL DEFAULT '' COMMENT '图片路径',
  `img_href` char(200) NOT NULL DEFAULT '' COMMENT '跳转页面',
  `create_time` timestamp NULL DEFAULT '0000-00-00 00:00:00' COMMENT '发表时间',
  `update_time` timestamp NOT NULL DEFAULT '0000-00-00 00:00:00' ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
  `article_extend` text CHARACTER SET utf8 COLLATE utf8_unicode_ci COMMENT '扩展信息',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='首页图片信息表';
