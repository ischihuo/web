<template>
    <div class="add-article">
        <el-form ref="form" :model="data" :rules="rules" label-width="80px">
            ...
            <el-form-item label="封面图片" prop="pic" class='upfile'>
                <div class="imgdiv">
                    <img v-show="data.art_img" :src="imgPath+data.art_img">
                </div>
                <el-upload
                    class="upload-demo"
                    drag
    ...
    module.exports = {
        ...
        methods: {
            handleAvatarSuccess(res, file) {
                this.data.art_img = res.data.filename;
            },
            saveArticle(){
                this.$refs.form.validate(v => {
                    if (v) {
                        this.visible = true;
                        this.data.article_extend = JSON.stringify(this.data.pic);
                        ajax.call(this, Action.UPDATEARTICLE, this.data, (data, err) => {
                            this.loading = false;
                            if (!err) {
                                this.$message({
                                    message: '保存成功',
                                ...
                                this.$router.push('/backg/article/list');
                            }
                        })
                    }
                });
