        ...
        methods: {
            batchesDelFiles(){
                if(this.multipleSelection.length){
                    ...
                    this.$confirm(`确定要批量删除文件和记录吗？`, '系统提醒', {
                        ...
                        ajax.call(this, Action.DELFILE, {id:ids,delRecord:true}, (d, err) => {
                            if (!err) {
                                this.ajaxData();
                            }
                        })
            ...
            headleClick(type, row){
                this.$confirm(`确定要删除文件${type==='delFile'?'':'和记录'}吗？`, '系统提醒', {
                    ...
                    ajax.call(this, Action.DELFILE, {id:row.id,delRecord:type === 'delFileAll'}, (d, err) => {
                        if (!err) {
                            if(type === 'delFileAll'){
                                this.ajaxData();
                            }else{
            ...
            ajaxData(){
                ajax.call(this, Action.LISTUPFILE, this.search_data, (obj, err) => {
                    if (!err) {
