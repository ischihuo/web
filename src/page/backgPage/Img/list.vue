        ...
        <el-row class="grid-table">
            <el-form :inline="true" :model='search_data'>
                ...
                <el-form-item>
                    ...
                    <el-button size="small" icon="plus" type="primary" @click="add" :disabled="grade.updateArticle">添加图片</el-button>
                </el-form-item>
            </el-form>
    ...
    module.exports = {
        ...
        data() {
            return {
                ...
                table_data: {
                    columns: [
                        {"key": "img_title", "name": "图片名称", minWidth: 150},
                        {"key": "img_description", "name": "图片描述", width: 120},
                        {"key": "img_path", "name": "图片路径", width: 120},
                        {"key": "img_href", "name": "跳转页面", width: 80},
        ...
        methods: {
            ...
            deleteArticle(arr){
                ...
                this.$confirm(`确定要${arr.length>1?'批量删除文章':'删除此文章'}吗？`, '系统提醒', {
                    ...
                    ajax.call(this, Action.DELETEIMG, {ids:arr.map(o=>o.id).join(",")}, (d, err) => {
                        !err && this.ajaxData();
                    })
                }).catch(() => {});
            },
            passedArticle(arr,pass){
                ...
                ajax.call(this, Action.PASSEDARTICLE, {ids:arr.map(o=>o.id).join(","),passed:pass}, (obj, err) => {
                    if (!err) {
                        arr.forEach(row=>{
            ...
            add(){
                this.$router.push('/backg/img/add');
            },
            ...
            healColumnClick(code, row){
                if(code ==='edit'){
                    this.$router.push('/backg/img/edit/'+row.id);
                }else if(code ==='view'){
            ...
            getActiveContent(id){
                this.loading = !0;
                ajax.call(this, 'Action.GETARTICLEBYID', {id}, (obj, err) => {
                    this.loading = !1;
                    if (err) {
            ...
            ajaxData(){
                ...
                ajax.call(this, Action.LISTIMG, this.search_data, (obj, err) => {
                    console.log(obj)
                    if (!err) {
